...
class callAndResponseCmds : public testing::TestWithParam<testParams>{
    ...
    callAndResponseCmds():  odrv("vcan0",axis_id){}
...
TEST_P(callAndResponseCmds,happy_case){
    GetFunction func = GetParam().func;
    int ans_size = GetParam().ans_size;
    int n_of_good_msgs = GetParam().n_of_good_msgs;
    std::vector<uint64_t> good_msgs = GetParam().good_msgs;
    ...
    case GetFunction::ENCODER_ERROR:
        ...
        memcpy(given_answer.data,&good_msgs[0],ans_size);
        EncoderError answer;
        break;
    ...
    std::thread wfmaa_thread(wait_for_msg_and_answer,
                expected,&given_answer, 1, false, &fixture_listening);
    test_sleep();
    ...
    case GetFunction::ENCODER_ERROR:
        ...
        memcpy(&expected_EncoderError,&good_msgs[0],ans_size);
...
INSTANTIATE_TEST_SUITE_P(Values,callAndResponseCmds,
    testing::Values(
        testParams{GetFunction::ENCODER_ERROR,4,{0},1}
    )
);
